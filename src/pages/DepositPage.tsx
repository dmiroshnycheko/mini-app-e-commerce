...
const DepositPage: React.FC<DepositPageProps> = ({ toggleTheme, isDarkMode }) => {
  ...
  const handleDeposit = () => {
    ...
    if (numAmount < 10) {
      toast.error(t('deposit.min_amount_error'));
      return;
    }
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Prevent non-numeric input (except for a single decimal point)
    if (!/^\d*\.?\d{0,2}$/.test(value)) {
      return;
    }

    // Remove leading zeros unless the value is '0' or starts with '0.'
  ...
  const handleBubbleClick = (value: number) => {
    setAmount(value.toFixed(2)); // Set bubble value with two decimal places
  };
